...
class KeyedHashSetTest extends TestCase
{
    public function testOperations(): void
    {
        ...
        $this->assertTrue($set->add($entry1));
        $this->assertFalse($set->isEmpty());
        $this->assertTrue($set->contains($entry1));
        $this->assertFalse($set->add($entry1));

        $set->add($entry2);
        $this->assertEquals([$entry1, $entry2], $set->values());
        $this->assertEquals(['2018-01-01' => $entry1, '2018-01-02' => $entry2], iterator_to_array($set));
        $this->assertCount(2, $set);

        $this->assertTrue($set->remove($entry1));
        $this->assertFalse($set->contains($entry1));
        $this->assertFalse($set->remove($entry1));
    ...
    public function testAccessByKey(): void
    {
        ...
        $this->assertTrue($set->containsKey($entry1->date));
        $this->assertTrue($set->containsKey($entry2->date));
        $this->assertFalse($set->containsKey(new DateTime('2018-01-03')));
        $this->assertEquals($entry1, $set->getByKey($entry1->date));
        $this->assertEquals($entry2, $set->fetchByKey($entry2->date));
        $this->assertNull($set->getByKey(new DateTime('2018-01-03')));

        $this->assertTrue($set->removeByKey($entry1->date));
        $this->assertFalse($set->containsKey($entry1->date));
        $this->assertFalse($set->removeByKey($entry1->date));
        $this->assertTrue($set->containsKey($entry2->date));
    }
    ...
    public function testFilter(): void
    {
        ...
        $this->assertEquals([$entry1], $filtered->values());
    }
}
